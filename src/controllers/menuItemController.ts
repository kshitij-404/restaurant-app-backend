...
export const getMenuItem = async (c: Context<{ params: { name: string } }>) => {
    ...
    try {
        const menuItems = await MenuItemModel.find({ restaurant: name });
        c.set.status = 200;
        return menuItems;
    } catch (error) {
        c.set.status = 500;
...
export const updateMenuItemStatus = async (c: Context<{ params: { id: string }, body: { isAvailable: boolean } }>) => {
