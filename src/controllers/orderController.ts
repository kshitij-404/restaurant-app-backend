...
export const createOrder = async (
  c: Context<{
    body: {
      items: { quantity: number; menuItemRef: string }[];
      orderMode: string;
    };
  }>
) => {
  ...
  try {
    ...
    const paymentData = {
      ...
      amount: detailedItems
        .reduce((total, item) => total + item.price * item.quantity, 0)
        .toFixed(1),
      p_info: detailedItems
...
export const getAllOrders = async (
  c: Context<{
    query: { status: "received" | "accepted" | "ready" | "delivered" };
  }>
) => {
  try {
    const orders = await OrderModel.find({
...
export const orderWebhook = async (c: Context) => {
  try {
    ...
    if (c.body.status !== "success") {
      await OrderModel
        // @ts-ignore
        .findByIdAndUpdate(new mongoose.Types.ObjectId(c.body.client_txn_id), {
...
export const poolingTxnStatus = async (
  ...
  try {
    ...
    const res = await ky.get(`https://qrstuff.me/gateway/check_txn/${id}`);
